...
| `MMO_DEBUG_NO_WORLD_SHADOWS=1` | disable the WHOLE directional-shadow path (u_shadowDim=1: no map compares, no facing-away darkening) — the no-shadow aliasing baseline for shimmer hunts: any camera-motion sparkle that survives this is sprite/texture resampling, not shadows |
| `MMO_SHIMMER_TEST=1` | alternates a ~half-pixel camera yaw offset per MMO_SHOT frame: shots 1,3,5,7 render at the base yaw, 2,4,6,8 at +0.0007 rad — so diff(N,N+1) isolates what a TINY camera move changes (temporal-aliasing/shimmer hunts) and diff(N,N+2) is the camera-identical control (animated content only). Combine with MMO_TIME_LOCK + MMO_PARTICLES=0 + MMO_NO_POST=1 for a clean signal; mouse motion can't be injected into a background GLFW window, this is the deterministic stand-in |
...
| `MMO_DEBUG_SHADOW=2` | PCF sampling internals view (the mode that cracked the block-seam lit-lines hunt): **R** = tap spread in texels beyond the 1-texel floor (/8), **G** = PCF lit fraction (since the shadow-AA batch this is the bilinear-weighted/tent fraction — smooth gradients at edges are CORRECT; dithered steps would mean the AA regressed), **B** = bias meters (/3); facing-away pixels (never sampled) render the EXACT-normal face id as a purple ramp (fid/8 in R and B) — every pixel of one block face must read a single flat value, so any speckle there means the face-id decode broke. Mirrors the main path's math exactly — read line/spike pixels vs face-interior pixels numerically off the PNG to attribute an artifact to spread vs bias vs branch flips |
