/**
 * Live probe for THE STRANGLER'S MARCH (world-redesign batch 4) — the L5-7
 * splice between the Kingless Wood and the Gloomfen:
 *   1. /room forest, /tp to the north gate: usePortal forest-march → the
 *      march, arriving at the TWIN GATE (beside march-forest, not room spawn).
 *   2. walk the BENDING road south gate → fen gate (waypoints follow the
 *      authored bends: the fork, the field-grid crossing, the north-west
 *      curl — the direct ray is under the murk). Wolves/boars/blooms along
 *      the road are L5-7; the probe is /level 30 and drinks through chip.
 *   3. usePortal march-gloomfen → the Gloomfen, arriving at ITS twin gate
 *      (gloomfen-march, the old forest gate's position, 160,308).
 *   4. returnToHub (H-key wire path) → hub.
 *   5. the Run's climb-out: /room greenhood_run, /tp to the East Door,
 *      usePortal greenhood-out → the ONE-WAY landing at the march's
 *      chute-mouth mound (28.5,148.5, standing ON the crown, y≈17; no
 *      return portal within 30 m).
 *   6. sanity: the Elder Strangler is alive at the farmstead (L8, mantrap
 *      sprite) and the strangler-gardens blooms resolve at L6.
 *
 * Needs admin: node scripts/make-admin.mjs march_probe   (then rerun)
 * Exits 0 on success, 1 on any failed expectation, 2 if not admin.
 *
 *   node scripts/march-probe.mjs
 */
import WebSocket from "ws";
import { loadEnv, makeWorldTracker, sleep, goTo } from "./lib.mjs";

loadEnv();
const MASTER = process.env.MMO_MASTER_ORIGIN ?? `http://127.0.0.1:${process.env.MASTER_PORT ?? 4000}`;

async function api(path, body, token) {
  const res = await fetch(`${MASTER}${path}`, {
    method: body ? "POST" : "GET",
    headers: { "content-type": "application/json", ...(token ? { authorization: `Bearer ${token}` } : {}) },
    body: body ? JSON.stringify(body) : undefined,
  });
  const json = await res.json();
  if (!res.ok) throw new Error(`${path}: ${json.error}`);
  return json;
}

const log = (...a) => console.log("[march]", ...a);
let failed = false;
function expect(cond, what) {
  if (cond) log(`OK   ${what}`);
  else { failed = true; log(`FAIL ${what}`); }
}

function enterRoom(wsUrl, ticket) {
  return new Promise((resolvePromise, reject) => {
    const ws = new WebSocket(wsUrl);
    const tracker = makeWorldTracker();
    const state = {
      roomId: null, selfId: -1, x: 0, y: 0, z: 0, seq: 0,
      terrain: null, portals: [], portalStates: [], transfer: null,
      ents: new Map(), stats: null, inv: null,
      events: [], chats: [], died: false, corrections: 0,
    };
    const timer = setTimeout(() => reject(new Error("room join timeout")), 25000);
    ws.on("open", () => ws.send(JSON.stringify({ t: "hello", v: 1, ticket })));
    ws.on("error", (e) => reject(e));
    ws.on("message", (raw) => {
      const msg = JSON.parse(raw.toString());
      switch (msg.t) {
        case "welcome":
          state.roomId = msg.roomId; state.selfId = msg.selfId;
          state.x = msg.spawn.x; state.y = msg.spawn.y; state.z = msg.spawn.z;
          for (const e of msg.ents) state.ents.set(e.id, e);
          break;
        case "world": case "chunks": case "blockSet": {
          const s = tracker.handle(msg);
          if (s) state.terrain = s;
          break;
        }
        case "portals": state.portals = msg.portals; break;
        case "portalState": state.portalStates.push(msg); break;
        case "snap":
          for (const e of msg.enter) state.ents.set(e.id, e);
          for (const d of msg.ents) { const e = state.ents.get(d.id); if (e) Object.assign(e, d); }
          for (const id of msg.leave) state.ents.delete(id);
          break;
        case "stats": state.stats = msg; break;
        case "inv": state.inv = msg; break;
        case "evt": state.events.push(msg.e); break;
        case "chat": state.chats.push(msg); break;
        case "died": state.died = true; break;
        case "transfer": state.transfer = msg; break;
        case "correct": state.x = msg.x; state.y = msg.y; state.z = msg.z; state.corrections++; break;
        case "reject": clearTimeout(timer); reject(new Error(`rejected: ${msg.reason}`)); return;
      }
      if (state.roomId && state.terrain && !state.ready) {
        state.ready = true;
        clearTimeout(timer);
        resolvePromise({ ws, state });
      }
    });
  });
}

async function waitTransfer(state, ms = 10000) {
  for (let i = 0; i < ms / 100; i++) {
    if (state.transfer) return state.transfer;
    await sleep(100);
  }
  return null;
}

async function gotoRoom(ws, state, roomId) {
  if (state.roomId === roomId) return { ws, state };
  let t = null;
  for (let attempt = 0; attempt < 3 && !t; attempt++) {
    state.transfer = null;
    ws.send(JSON.stringify({ t: "chat", text: `/room ${roomId}` }));
    t = await waitTransfer(state);
    if (!t) await sleep(2000);
  }
  if (!t) throw new Error(`/room ${roomId}: no transfer granted (admin? room open?)`);
  ws.close();
  return enterRoom(t.wsUrl, t.ticket);
}

async function tp(ws, state, x, z) {
  ws.send(JSON.stringify({ t: "chat", text: `/tp ${x} ${z}` }));
  for (let i = 0; i < 40; i++) {
    await sleep(100);
    if (Math.hypot(state.x - x, state.z - z) < 3) return true;
  }
  return false;
}

/** Waypoint walk with a potion guard — the march is a live L5-7 room and the
 *  road passes its wolf verge; a probe that never drinks bleeds out to chip. */
async function walkRoad(ws, state, points) {
  for (const [wx, wz] of points) {
    if (state.stats && state.stats.hp < 400) {
      const pot = (state.inv?.slots ?? []).findIndex((s) => s && s.item === "greater_health_potion");
      if (pot < 0) ws.send(JSON.stringify({ t: "chat", text: "/give greater_health_potion 10" }));
      else ws.send(JSON.stringify({ t: "consume", slot: pot }));
      await sleep(900);
    }
    const ok = await goTo(ws, state, wx, wz, 1.6);
    if (!ok) {
      log(`     (stalled at ${state.x.toFixed(1)},${state.z.toFixed(1)} heading for ${wx},${wz})`);
      return false;
    }
  }
  return true;
}

// ---- login as the admin probe bot ----
await api("/api/register", { username: "march_probe", password: "march123" }).catch(() => {});
const { token, account } = await api("/api/login", { username: "march_probe", password: "march123" });
if (!account.roles.includes("admin")) {
  console.error("[march] march_probe is not admin — run: node scripts/make-admin.mjs march_probe  (then rerun)");
  process.exit(2);
}
let { characters } = await api("/api/characters", null, token);
if (!characters.length) characters = [(await api("/api/characters", { name: "Marchprobe" }, token)).character];
const grant = await api("/api/enter", { characterId: characters[0].id }, token);
let { ws, state } = await enterRoom(grant.wsUrl, grant.ticket);
log(`entered ${state.roomId} at ${state.x.toFixed(1)},${state.z.toFixed(1)}`);
ws.send(JSON.stringify({ t: "chat", text: "/level 30" }));
ws.send(JSON.stringify({ t: "chat", text: "/give greater_health_potion 10" }));

// ---- 1. hub → forest → the march, twin-gate arrival ----
({ ws, state } = await gotoRoom(ws, state, "forest"));
for (let i = 0; i < 30 && state.portals.length === 0; i++) await sleep(100);
const northGate = state.portals.find((p) => p.id === "forest-march");
expect(!!northGate && northGate.target === "stranglers_march", "forest north gate targets the march");
expect(!!northGate && northGate.open === true, "the march gate is always open (a thoroughfare, no gating)");
await tp(ws, state, 240, 35);
await goTo(ws, state, 240, 31.5, 1.2);
state.transfer = null;
ws.send(JSON.stringify({ t: "usePortal", portalId: "forest-march" }));
const t1 = await waitTransfer(state);
expect(!!t1, "forest-march grants a transfer");
if (!t1) { log("RESULT: FAIL"); process.exit(1); }
ws.close();
({ ws, state } = await enterRoom(t1.wsUrl, t1.ticket));
expect(state.roomId === "stranglers_march", `arrived in ${state.roomId}`);
const dGate = Math.hypot(state.x - 120, state.z - 228);
expect(dGate < 6, `twin-gate arrival beside march-forest (${state.x.toFixed(1)},${state.z.toFixed(1)}, ${dGate.toFixed(1)}m from the gate)`);

// ---- 2. walk the bending road (the direct ray is under the murk) ----
const ROAD = [
  [120, 198],
  [132, 176],
  [134, 136],
  [136, 110],
  [124, 84],
  [104, 64],
  [84, 40],
  [84, 28],
];
const walked = await walkRoad(ws, state, ROAD);
expect(walked, `walked the bending road to the fen gate (${state.x.toFixed(1)},${state.z.toFixed(1)}; ${state.corrections} corrections)`);

// ---- 3. the fen gate → gloomfen twin-gate arrival ----
await goTo(ws, state, 84, 25.2, 0.9); // step inside the arch's trigger radius
for (let i = 0; i < 30 && state.portals.length === 0; i++) await sleep(100);
expect(state.portals.some((p) => p.id === "march-gloomfen" && p.open), "march-gloomfen is open");
state.transfer = null;
let t2 = null;
for (let attempt = 0; attempt < 3 && !t2; attempt++) {
  ws.send(JSON.stringify({ t: "usePortal", portalId: "march-gloomfen" }));
  t2 = await waitTransfer(state, 5000);
}
expect(!!t2, "march-gloomfen grants a transfer");
if (!t2) { log("RESULT: FAIL"); process.exit(1); }
ws.close();
({ ws, state } = await enterRoom(t2.wsUrl, t2.ticket));
expect(state.roomId === "gloomfen", `arrived in ${state.roomId}`);
const dFen = Math.hypot(state.x - 160, state.z - 308);
expect(dFen < 6, `twin-gate arrival beside gloomfen-march (${state.x.toFixed(1)},${state.z.toFixed(1)}, ${dFen.toFixed(1)}m from the gate)`);
for (let i = 0; i < 30 && state.portals.length === 0; i++) await sleep(100);
expect(state.portals.some((p) => p.id === "gloomfen-march" && p.target === "stranglers_march"), "gloomfen's south gate targets the march back");

// ---- 4. returnToHub (the H-key wire path) ----
state.transfer = null;
ws.send(JSON.stringify({ t: "returnToHub" }));
const t3 = await waitTransfer(state);
expect(!!t3, "returnToHub grants a transfer");
if (!t3) { log("RESULT: FAIL"); process.exit(1); }
ws.close();
({ ws, state } = await enterRoom(t3.wsUrl, t3.ticket));
expect(state.roomId === "hub", `home in ${state.roomId}`);

// ---- 5. the Run's climb-out lands on the march's chute-mouth mound ----
({ ws, state } = await gotoRoom(ws, state, "greenhood_run"));
await tp(ws, state, 80, 15);
await goTo(ws, state, 80.5, 13.2, 0.9);
state.transfer = null;
let t4 = null;
for (let attempt = 0; attempt < 3 && !t4; attempt++) {
  ws.send(JSON.stringify({ t: "usePortal", portalId: "greenhood-out" }));
  t4 = await waitTransfer(state, 5000);
}
expect(!!t4, "the East Door grants a transfer");
if (!t4) { log("RESULT: FAIL"); process.exit(1); }
ws.close();
({ ws, state } = await enterRoom(t4.wsUrl, t4.ticket));
expect(state.roomId === "stranglers_march", `climbed out into ${state.roomId}`);
expect(Math.hypot(state.x - 28.5, state.z - 148.5) < 1.5, `landed at the authored mound (${state.x.toFixed(1)},${state.z.toFixed(1)})`);
expect(Math.abs(state.y - 17) < 1.5, `standing ON the mound crown (y=${state.y.toFixed(1)})`);
for (let i = 0; i < 30 && state.portals.length === 0; i++) await sleep(100);
expect(!state.portals.some((p) => Math.hypot(p.x - 28.5, p.z - 148.5) < 30), "no return portal at the mound (one-way by omission)");

// ---- 6. the Elder holds its farmstead ----
await tp(ws, state, 153, 122);
await sleep(1200);
const elder = [...state.ents.values()].find((e) => e.kind === "mob" && e.name === "The Elder Strangler" && e.act !== "dead");
expect(!!elder, `the Elder Strangler is alive at its lair ${elder ? `(${elder.x.toFixed(0)},${elder.z.toFixed(0)}, L${elder.level}, hp ${elder.hp}/${elder.maxHp})` : ""}`);
if (elder) {
  expect(elder.level === 8, `the Elder is L8 (band-top+1), got L${elder.level}`);
  // rooted-ish: moveSpeed 0.6 lets it drift within its shell (the mantrap
  // pattern), but leash 18 + the r5 table keep it AT the farmstead
  expect(Math.hypot(elder.x - 153, elder.z - 105) < 14, `it holds the farmstead (${Math.hypot(elder.x - 153, elder.z - 105).toFixed(1)}m from the heart)`);
}
const bloom = [...state.ents.values()].find((e) => e.kind === "mob" && e.name === "Strangler Bloom" && e.act !== "dead");
expect(!!bloom && bloom.level === 6, `the garden's blooms run at L6 ${bloom ? `(saw L${bloom.level})` : "(none in interest)"}`);

ws.close();
log(failed ? "RESULT: FAIL" : "RESULT: PASS");
process.exit(failed ? 1 : 0);
