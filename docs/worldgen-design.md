...
**Item icons**: `items.json` `icon: [col,row]` indexes `client/assets/ui/icons.png` = `IconSet/tficons_limited_16.png` verbatim (**16 cols × 64 rows**, 256×1024) **plus one appended row (row 64)** where block items get their tile as icon (currently cols 0–13 used; **cols 14–15 free** = 2 more block items before a second appended row is needed). There is no 32px sheet: the first-person viewmodel EXTRUDES the 16px icon into a 3D mesh (`ItemMeshes.java`), so new items need zero extra art work. Verify any icon change with `node tools/verify-icons.mjs` (machine checks) + `node tools/icon-proof.mjs` (look at it).
...
Baselines: rusty_sword 6 dmg/250 dur/8 g → iron_sword 11/450/40 → longbow 14/500/60. **T2 (Gloomfen, L8–12) ≈ 1.5× iron; T3 (Cinderrift/Depths, L12–15) ≈ 2.2×.** All icons live on tficons_limited_16.png (rows 27-31 blades/axes/spears/staves, rows 33-34 bows/crossbows, row 17 flasks, rows 36-47 shields/armour, rows 55-57 trophies).
